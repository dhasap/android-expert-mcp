...
function scoreToPercent(score: number | null): string {
  if (score === null) return "N/A";
  return `${Math.round(score * 100)}`;
}

function parseLighthouseReport(lhResult: LighthouseResult): string {
  ...
  const failedAudits: Array<{ title: string; description: string; score: number | null; displayValue?: string }> = [];
  const warningAudits: typeof failedAudits = [];
  ...
  if (failedAudits.length > 0) {
    ...
    for (const a of failedAudits.slice(0, 15)) {
      ...
      if (a.displayValue) sections.push(`**Value**: ${a.displayValue}`);
  ...
  const a11yAudits = Object.values(audits)
    .filter((a) => {
      ...
      return (
        cat?.auditRefs?.some((ref) => ref.id === a.id) &&
        a.score !== null &&
        a.score < 1
      );
    })
...
export function registerAuditTools(server: McpServer): void {
  ...
  server.tool(
    "take_screenshot",
    "Takes a full-page screenshot of a website using Puppeteer. " +
      ...
      "Supports both desktop and mobile viewport emulation.",
    {
      ...
      output_path: z
        ...
        .optional()
        .describe(
          "Where to save the PNG file. Defaults to a timestamped file in /tmp/mcp-screenshots/"
        ),
      full_page: z
        .boolean()
        .default(true)
    ...
    async ({ url, output_path, full_page, device, wait_seconds, timeout_seconds }) => {
      ...
      try {
        ...
        const finalPath =
          output_path ?? path.join(screenshotDir, `${safeDomain}_${device}_${timestamp}.png`);
        const resolvedPath = path.resolve(finalPath);
        await ensureDir(path.dirname(resolvedPath));

        await page.screenshot({
          path: resolvedPath as `${string}.png`,
          fullPage: full_page,
          type: "png",
        });
  ...
  server.tool(
    ...
    "Runs a comprehensive Lighthouse audit on a website, measuring Performance, " +
      ...
      "with scores and detailed findings. Also saves the raw JSON report.",
    {
      ...
      device: z
        ...
        .default("mobile")
        .describe(
          "Emulation preset — Lighthouse defaults to mobile (default: mobile)"
        ),
      output_dir: z
        .string()
        .optional()
        .describe(
          "Directory to save the JSON report. Defaults to /tmp/mcp-audits/"
        ),
      timeout_seconds: z
        .number()
        .int()
      ...
      try {
            ...
            content: [
              {
                type: "text",
                text:
                  "❌ Lighthouse not installed or not importable.\n" +
        ...
        const runnerResult = await Promise.race([
          ...
          new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error(`Lighthouse timed out after ${timeout_seconds}s`)), timeout_seconds * 1000)
          ),
        ]);
  ...
  server.tool(
    "parse_audit_report",
    "Reads a saved Lighthouse JSON report from disk and parses it into a " +
    ...
    async ({ report_path, focus }) => {
      try {
        ...
        const lhr = JSON.parse(rawJson) as LighthouseResult;
        ...
        const header =
          `📄 Parsed Audit Report: ${resolvedPath}\n` +
          `   Focus: ${focus}\n` +
          `${"═".repeat(60)}\n\n`;
  ...
  server.tool(
    ...
    "Checks a website's mobile responsiveness by loading it at multiple " +
      ...
      try {
        ...
        for (const vp of viewports) {
          ...
          try {
            ...
            const analysis = await page.evaluate((vpWidth: number) => {
  ...
  server.tool(
    "extract_seo_data",
    "Performs a deep SEO analysis of a webpage: checks title, meta description, " +
      ...
      try {
        ...
        const seoData = await page.evaluate(() => {
          ...
          const internalLinks = allLinks.filter((a) =>
            (a as HTMLAnchorElement).href.includes(domain)
          ).length;
          const externalLinks = allLinks.length - internalLinks;
          const nofollowLinks = allLinks.filter((a) =>
            (a as HTMLAnchorElement).rel?.includes("nofollow")
          ).length;
        ...
        else if (seoData.titleLength < 30) issues.push(`⚠️ Title too short (${seoData.titleLength} chars, aim for 50–60)`);
        else if (seoData.titleLength > 60) issues.push(`⚠️ Title too long (${seoData.titleLength} chars, aim for 50–60)`);
        else good.push(`✅ Title length OK (${seoData.titleLength} chars)`);

        if (!seoData.metaDescription) issues.push("❌ Missing meta description");
        else if (seoData.metaDescriptionLength < 120) issues.push(`⚠️ Meta description too short (${seoData.metaDescriptionLength} chars)`);
        else if (seoData.metaDescriptionLength > 160) issues.push(`⚠️ Meta description too long (${seoData.metaDescriptionLength} chars)`);
        else good.push(`✅ Meta description OK (${seoData.metaDescriptionLength} chars)`);

        if (!seoData.canonical) issues.push("⚠️ No canonical URL set");
        else good.push("✅ Canonical URL present");

        if ((seoData.headings.h1 ?? []).length === 0) issues.push("❌ No H1 tag found!");
        else if ((seoData.headings.h1 ?? []).length > 1) issues.push(`⚠️ Multiple H1 tags (${(seoData.headings.h1 ?? []).length})`);
        else good.push("✅ Single H1 tag");

        if (seoData.imagesWithoutAlt > 0) issues.push(`⚠️ ${seoData.imagesWithoutAlt}/${seoData.totalImages} images missing alt text`);
        else if (seoData.totalImages > 0) good.push(`✅ All ${seoData.totalImages} images have alt text`);

        if (!seoData.ogTitle) issues.push("⚠️ Missing OG title (og:title)");
        else good.push("✅ Open Graph title set");

        if (!seoData.ogImage) issues.push("⚠️ Missing OG image (og:image)");
        if (!seoData.twitterCard) issues.push("⚠️ Missing Twitter Card meta");
        if (!seoData.viewport) issues.push("❌ Missing viewport meta tag");
        if (!seoData.lang) issues.push("⚠️ Missing lang attribute on <html>");
        if (seoData.wordCount < 300) issues.push(`⚠️ Low word count (${seoData.wordCount} words)`);
        if (seoData.jsonld.length > 0) good.push(`✅ Structured data found (${seoData.jsonld.length} JSON-LD block(s))`);
        else issues.push("⚠️ No structured data (JSON-LD) found");
        ...
        const report = [
          ...
          seoData.jsonld.length > 0
            ? seoData.jsonld.map((j: Record<string, unknown>) => `  • @type: ${j?.["@type"] ?? "unknown"}`).join("\n")
            : `  NONE`,
          ``,
          `${"─".repeat(60)}`,
          `✅ PASSING (${good.length}):`,
