...
export function registerArchitectureTools(server: McpServer): void {
  ...
  server.tool(
    ...
    "Reads and returns the directory tree of a project folder. " +
    ...
    async ({ project_path, max_depth }) => {
      try {
        ...
        const output =
          `📂 Project Structure: ${resolvedPath}\n` +
          `${"─".repeat(60)}\n` +
  ...
  server.tool(
    "read_file",
    "Reads the content of any file (source code, markdown, configs, etc.). " +
  ...
  server.tool(
    "edit_file",
    "Edits an existing file by replacing a specific substring with new content. " +
            ...
            content: [
              {
                type: "text",
                text:
                  `ERROR [edit_file]: search_text not found in '${resolvedPath}'.\n` +
  ...
  server.tool(
    "list_files",
    "Lists all files in a directory matching an optional glob-like extension filter.",
    {
      ...
      try {
        ...
        async function collectFiles(dir: string, depth: number): Promise<string[]> {
          ...
          for (const entry of entries) {
            if (["node_modules", ".git", ".gradle", "build"].includes(entry.name)) continue;
