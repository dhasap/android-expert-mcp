...
async function retryWithBackoff<T>(
  fn: () => Promise<T>,
  options: { maxRetries?: number; retryDelay?: number; operation?: string } = {}
): Promise<T> {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      process.stderr.write(
        `[browser] ${operation}: attempt ${attempt}/${maxRetries} failed, retrying in ${waitTime}ms...\n`
      );
...
async function evictOldestIdleSession(): Promise<string | null> {
  ...
  if (!evicted) return null; // Race condition: session already deleted
...
export function registerBrowserTools(server: McpServer): void {
  ...
  server.tool(
    "browser_open",
    "Buka browser baru dan navigasi ke URL. Simpan session_id untuk operasi selanjutnya." +
      ...
      try {
        ...
        if (!session) {
          ...
          page.on("dialog", async (dialog) => {
            try {
              ...
              process.stderr.write(`[browser] Dialog handle error: ${(dialogError as Error).message}\n`);
            }
          });
        ...
        if (doScreenshot) {
          try {
            ...
            screenshotInfo = `\n📸 Screenshot: Failed - ${(ssError as Error).message}`;
          }
        }
  ...
  server.tool(
    ...
    async ({ session_id, full_page, label, timeout_seconds }) => {
      try {
        ...
        const takeScreenshotWithTimeout = async (): Promise<void> => {
          const screenshotPromise = page.screenshot({
            path: filePath as `${string}.png`,
            fullPage: full_page,
            type: "png",
          ...
          content: [
            {
              type: "text",
              text:
                `📸 Screenshot diambil! [STABILIZED v5.2]\n` +
                `${"─".repeat(50)}\n` +
                `${pageInfo}\n` +
  ...
  server.tool(
    "browser_click",
    "Klik elemen di halaman menggunakan CSS selector. Auto-scroll dan screenshot verifikasi." +
    ...
    async ({ 
      ...
      try {
        ...
        if (!clickSuccess && fallback_js) {
          try {
            const jsResult = await page.evaluate(JS_CLICK_SCRIPT(selector)) as { success?: boolean; text?: string; tagName?: string };
            if (jsResult?.success) {
              clickSuccess = true;
          ...
          content: [
            {
              type: "text",
              text:
                `✅ Navigasi '${action}' berhasil! [STABILIZED v5.2]\n` +
                `${"─".repeat(50)}\n` +
                pageInfo +
                recreatedMsg +
                ssInfo,
              ...
              content: [
                {
                  type: "text",
                  text:
                    `✅ Navigasi '${action}' berhasil dengan session recovery! [STABILIZED v5.2]\n` +
                    `${"─".repeat(50)}\n` +
          ...
          content: [
            {
              type: "text",
              text:
                `📄 Content (${content_type}) [STABILIZED v5.2]\n` +
                `${"─".repeat(50)}\n` +
                `${pageInfo}\n\n` +
                `${"─".repeat(50)}\n` +
                output,
          ...
          content: [
            {
              type: "text",
              text:
                `⚡ Script dieksekusi! [STABILIZED v5.2]\n` +
                `${"─".repeat(50)}\n` +
