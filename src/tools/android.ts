...
async function resolveGradlew(projectPath: string): Promise<string> {
  const candidates = [
  ...
  throw new Error(
    `gradlew wrapper not found in '${projectPath}'. ` +
...
function deviceFlag(deviceSerial?: string): string {
...
export function registerAndroidTools(server: McpServer): void {
  ...
  server.tool(
    "run_gradle_task",
    "Executes a Gradle task using the project's ./gradlew wrapper. " +
  ...
  server.tool(
    "read_build_log",
    "Reads a build log file from disk (or a specified path) and intelligently " +
      ...
      "Use this for post-mortem analysis of CI logs or saved build outputs.",
    {
      ...
      extract_only_errors: z
        ...
        .default(true)
        .describe(
          "If true, returns only extracted errors/stack traces. " +
          ...
          content: [
            {
              type: "text",
              text:
                `📋 Build Log Analysis: ${resolvedPath}\n` +
  ...
  server.tool(
    "adb_list_devices",
    "Lists all currently connected Android devices/emulators via ADB. " +
      "Returns device serials, states, and model info if available.",
    {},
    async () => {
      try {
            ...
            content: [
              {
                type: "text",
                text:
                  `❌ ADB not available: ${adbCheck.error}\n` +
  ...
  server.tool(
    "adb_dump_ui",
    "Dumps the current UI hierarchy of a connected Android device using " +
  ...
  server.tool(
    "adb_read_logcat",
    "Captures Android logcat output for a specified duration. " +
      ...
      try {
        ...
        if (package_name && logOutput) {
          const lines = logOutput.split("\n").filter(
            (line) => line.includes(package_name) || line.startsWith("---") || line.length < 10
          );
          ...
          content: [
            {
              type: "text",
              text:
                `📋 Logcat Output (${duration_seconds}s capture, level >= ${level})\n` +
  ...
  server.tool(
    "adb_extract_apk",
    "Extracts (pulls) the APK of an installed app from a connected Android device. " +
    ...
    async ({ package_name, output_dir, device_serial }) => {
      try {
        ...
        const pathResult = await runAdbCommand(
          `adb ${flag} shell pm path "${safePackageName}"`,
          undefined,
          15_000
        );
            ...
            content: [
              {
                type: "text",
                text:
                  `❌ Package '${package_name}' not found on device.\n` +
        ...
        const pullResult = await runAdbCommand(
          `adb ${flag} pull "${shellEscape(remotePath)}" "${shellEscape(localPath)}"`,
          undefined,
          60_000
        );
  ...
  server.tool(
    "adb_run_shell",
    "Executes an arbitrary shell command on a connected Android device via ADB. " +
  ...
  server.tool(
    "analyze_kotlin_file",
    "Reads a Kotlin source file and provides a structural analysis: " +
    ...
    async ({ file_path }) => {
      try {
        ...
        const smells: string[] = [];
        if (lines.length > 500) smells.push(`⚠️  File is very long (${lines.length} lines) — consider splitting`);
        if (functions.length > 20) smells.push(`⚠️  Too many functions (${functions.length}) — consider refactoring`);
        if (imports.length > 30) smells.push(`⚠️  Many imports (${imports.length}) — possible god class`);
        if (todoCount > 0) smells.push(`⚠️  Found ${todoCount} TODO/FIXME comment(s)`);

        const output =
          `🔍 Kotlin File Analysis: ${resolvedPath}\n` +
          `${"─".repeat(60)}\n` +
          `Package    : ${packageName}\n` +
          `Lines      : ${lines.length}\n` +
