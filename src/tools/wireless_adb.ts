 ...
 */
async function listWirelessDevices(): Promise<
  Array<{ address: string; state: string }>
> {
 ...
 */
function deviceFlag(address?: string): string {
  return address ? `-s ${address}` : "";
}

// ─── Tool registration ────────────────────────────────────────────────────────

export function registerWirelessAdbTools(server: McpServer): void {
  // ── 1. adb_wifi_pair ─────────────────────────────────────────────────────
  server.tool(
    "adb_wifi_pair",
    "Pasangkan (pair) perangkat Android 11+ via wireless debugging tanpa kabel USB. " +
      ...
      "Hanya perlu dilakukan SEKALI per perangkat — selanjutnya cukup adb_wifi_connect.",
    {
      pair_address: z
        .string()
        .describe(
          "HOST:PORT untuk pairing (berbeda dari port koneksi!). " +
    ...
    async ({ pair_address, pairing_code }) => {
      try {
        const result = await runAdbCommand(
          `adb pair ${pair_address} ${pairing_code}`,
          undefined,
          30_000
        );
        ...
        const success =
          /successfully paired/i.test(combined) || result.exitCode === 0;
          ...
          content: [
            {
              type: "text",
              text:
                `${success ? "✅" : "❌"} ADB Pairing — ${pair_address}\n` +
                `${"─".repeat(55)}\n` +
                `${combined}\n\n` +
                (success
                  ? `🎉 Pairing berhasil!\n` +
  ...
  server.tool(
      ...
      "Untuk Android 10-: sambungkan USB dulu, jalankan adb_wifi_enable, cabut USB, baru connect.",
    {
      address: z
        .string()
        .describe(
          "IP:PORT perangkat. Contoh: '192.168.1.5:5555' (Android 10-) " +
    ...
    async ({ address }) => {
      try {
        ...
        const combined = (result.stdout + result.stderr).trim();
        const connected =
          /connected to/i.test(combined) || /already connected/i.test(combined);
          ...
          content: [
            {
              type: "text",
              text:
                `${connected ? "✅" : "❌"} ADB WiFi Connect — ${address}\n` +
                `${"─".repeat(55)}\n` +
                `${combined}\n\n` +
  ...
  server.tool(
    "adb_wifi_enable",
    "Aktifkan mode TCP/IP pada perangkat yang SAAT INI tersambung via USB (Android 10-). " +
      ...
      "Untuk Android 11+, fitur ini tidak diperlukan — gunakan Wireless Debugging langsung.",
    {
      ...
      device_serial: z
        ...
        .optional()
        .describe(
          "Serial USB device jika ada lebih dari satu. Kosongkan jika hanya ada satu device."
        ),
    },
    async ({ port, device_serial }) => {
      try {
        ...
        const ipResult = await runAdbCommand(
          `adb ${flag} shell ip addr show wlan0 | grep 'inet '`,
          undefined,
          8_000
        );
  ...
  server.tool(
    "adb_wifi_devices",
    "Tampilkan semua perangkat Android yang terhubung via wireless ADB (TCP/IP). " +
      "Hanya menampilkan koneksi nirkabel, bukan USB.",
    {},
    async () => {
      try {
        const devices = await listWirelessDevices();
        ...
        lines.push("");
        lines.push(
          `💡 Gunakan address (misal "${devices[0]?.address}") sebagai parameter 'address' di tool lain.`
        );
  ...
  server.tool(
    "adb_wifi_disconnect",
    "Putuskan koneksi wireless ADB dari perangkat. " +
      "Kosongkan 'address' untuk memutus SEMUA koneksi wireless sekaligus.",
    {
      address: z
        ...
        .optional()
        .describe(
          "IP:PORT yang ingin diputus. Kosongkan untuk disconnect semua wireless device."
        ),
    },
    async ({ address }) => {
      try {
  ...
  server.tool(
    "adb_wifi_shell",
    "Jalankan shell command di perangkat Android yang terhubung via wireless ADB. " +
      ...
      "Cocok untuk inspeksi app data, membaca log, cek permission, dll.",
    {
      address: z
        .string()
        .describe(
          "IP:PORT perangkat wireless. Contoh: '192.168.1.5:5555'. " +
            "Lihat dari adb_wifi_devices."
        ),
      command: z
        .string()
        .describe(
          "Shell command yang dijalankan. Contoh: 'pm list packages', " +
  ...
  server.tool(
    "adb_wifi_screenshot",
    "Ambil screenshot layar perangkat Android via wireless ADB. " +
      "Berguna untuk memverifikasi UI app, debug layout, atau mendokumentasikan state app.",
    {
      ...
      output_path: z
        ...
        .optional()
        .describe(
          "Path lokal untuk menyimpan PNG. Default: /tmp/mcp-emulator/wifi_ss_<ts>.png"
        ),
      display_id: z
        .number()
        .int()
    ...
    async ({ address, output_path, display_id }) => {
      try {
        ...
        const ts = Date.now();
        const localPath =
          output_path ?? path.join(outDir, `wifi_ss_${address.replace(/[:.]/g, "_")}_${ts}.png`);
        ...
        // Capture on device
        await runAdbCommand(
          `adb -s ${address} shell screencap -p ${displayFlag} ${remotePath}`,
          undefined,
          15_000
        );
        ...
        // Pull to local
        await runAdbCommand(
          `adb -s ${address} pull ${remotePath} "${localPath}"`,
          undefined,
          15_000
        );
        ...
        // Cleanup remote
        await runAdbCommand(
          `adb -s ${address} shell rm ${remotePath}`,
          undefined,
          5_000
        ).catch(() => null);
  ...
  server.tool(
    "adb_wifi_ui_dump",
    "Dump UI hierarchy (struktur widget) dari app yang sedang berjalan via wireless ADB. " +
      ...
      "Sangat berguna sebagai alternatif web scraping untuk app native.",
    {
      ...
      output_path: z
        ...
        .optional()
        .describe(
          "Path lokal untuk menyimpan XML dump. " +
      ...
      include_raw_xml: z
        ...
        .default(false)
        .describe(
          "Sertakan raw XML dalam response (bisa sangat panjang). " +
      ...
      filter_package: z
        ...
        .optional()
        .describe(
          "Filter hanya elemen dari package tertentu. " +
    ...
    async ({ address, output_path, include_raw_xml, filter_package }) => {
      try {
        ...
        const dumpResult = await runAdbCommand(
          `adb -s ${address} shell uiautomator dump --compressed ${remotePath}`,
          undefined,
          30_000
        );
        ...
        if (filter_package) {
          ...
          const pkgRegex = new RegExp(
            `package="${filter_package.replace(/\./g, "\\.")}[^"]*"[^>]*text="([^"]+)"`,
            "g"
          );
