...
export function registerGithubTools(server) {
    ...
    // ── 5. github_file_write ────────────────────────────────────────────────
    server.tool("github_file_write", "Buat/update file di GitHub via API (tanpa git lokal). Butuh GITHUB_TOKEN." +
        "[STABILIZED v5.2] Dengan auto-retry untuk network failures.", {
        repo: z.string().describe("Nama repository"),
    ...
    server.tool("github_commit_push", "Push beberapa file dalam satu commit ke GitHub via Tree API." +
        "[STABILIZED v5.2] Dengan auto-retry untuk network failures.", {
        repo: z.string().describe("Nama repository"),
        files: z
            .array(z.object({
