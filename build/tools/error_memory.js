...
async function loadBank() {
    ...
    return atomicReadJson(BANK_FILE, { ...BANK_DEFAULT, created_at: new Date().toISOString(), updated_at: new Date().toISOString() });
}
...
function fingerprintError(normalized) {
...
function findSimilar(bank, normalized, fingerprint, limit = 5) {
    return bank.entries
        .map((entry) => {
        let score = 0;
...
// ─── Tool Registration ────────────────────────────────────────────────────────
export function registerErrorMemoryTools(server) {
    // ── 1. error_remember ────────────────────────────────────────────────────
    server.tool("error_remember", "Simpan error baru ke Memory Bank. AI akan otomatis parse tech stack, " +
        ...
        try {
            ...
            return await withBank(async (bank) => {
                ...
                const similar = findSimilar(bank, normalized, fingerprint, 3).filter((e) => e.fingerprint !== fingerprint);
                // Buat entri baru
                const newEntry = {
    ...
    // ── 2. error_search ───────────────────────────────────────────────────────
    server.tool("error_search", "Cari error di Memory Bank berdasarkan keyword, tech stack, atau status. " +
        ...
        try {
            ...
            // Filter tags
            if (tags.length > 0) {
                results = results.filter((e) => tags.some((t) => e.tags.includes(t.toLowerCase())));
            }
            ...
            if (query) {
                ...
                results = results
                    .map((e) => ({
                    entry: e,
                    score: e.fingerprint === fingerprint
                        ? 1.0
                ...
                results = results.sort((a, b) => new Date(b.last_seen).getTime() - new Date(a.last_seen).getTime());
            }
    ...
    server.tool("error_add_solution", "Tambahkan solusi ke error yang ada di Memory Bank. " +
        ...
        try {
            return await withBank(async (bank) => {
                ...
                const entry = bank.entries.find((e) => e.id === error_id || e.id.startsWith(error_id));
                if (!entry) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: `❌ Error dengan ID '${error_id}' tidak ditemukan.\n` +
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: `${worked ? "✅" : "❌"} Solusi ditambahkan!\n` +
                                "═".repeat(55) +
                                `\nError  : ${entry.title}\n` +
                                `Status : ${entry.status}\n` +
    ...
    server.tool("error_auto_diagnose", "Analisis error baru secara otomatis: parse, cari di Memory Bank, " +
    ...
    // ── 5. error_stats ────────────────────────────────────────────────────────
    server.tool("error_stats", "Tampilkan statistik Memory Bank: error paling sering, " +
        ...
        try {
            ...
            if (project_filter) {
                entries = entries.filter((e) => e.project_context?.toLowerCase().includes(project_filter.toLowerCase()));
            }
            if (entries.length === 0) {
                return {
            ...
            const byStack = entries.reduce((acc, e) => {
                if (!acc[e.tech_stack])
                    acc[e.tech_stack] = { count: 0, solved: 0, total_occ: 0 };
                acc[e.tech_stack].count++;
                if (e.status === "solved")
                    acc[e.tech_stack].solved++;
            ...
            const healthEmoji = healthPct >= 80 ? "🟢" : healthPct >= 50 ? "🟡" : "🔴";
            const lines = [
            ...
            Object.entries(byStack)
                ...
                lines.push(`   ${stack.padEnd(12)} [${bar}] ${solveRate}% solved | ${data.count} unique | ${data.total_occ}x occurred`);
            });
            lines.push("");
            lines.push("🔁 TOP RECURRING ERRORS:");
            topRecurring.forEach((e, i) => {
                lines.push(`   ${i + 1}. (${e.occurrences}x) ${e.title.slice(0, 60)} [${e.status}]`);
            });
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (error) {
    ...
    server.tool("error_export", "Export Memory Bank ke file JSON untuk backup atau sharing antar device.", {
        output_path: z
            .string()
            .optional()
...
//# sourceMappingURL=error_memory.js.map