...
function renderSingleChoice(title, question, options, descriptions) {
    const width = 60;
...
function renderMultiChoice(title, question, options) {
    const width = 60;
    const border = "─".repeat(width);
    const lines = [
        `╭${border}╮`,
...
function renderConfirm(question, detail) {
    const width = 60;
    const border = "─".repeat(width);
    return [
        `╭${border}╮`,
...
function renderProgress(title, steps, currentMessage) {
    const statusIcon = {
        done: "✅",
        active: "⏳",
        pending: "⬜",
...
function renderInfoCard(title, fields, footer) {
    const width = 60;
    const border = "─".repeat(width);
    const lines = [
        `╭${border}╮`,
...
export function registerInteractiveTools(server) {
    // ── 1. ui_single_choice ───────────────────────────────────────────────────
    server.tool("ui_single_choice", "Tampilkan UI pilihan tunggal (radio button style) ke user. " +
    ...
    // ── 2. ui_multi_choice ────────────────────────────────────────────────────
    server.tool("ui_multi_choice", "Tampilkan UI pilihan berganda (checkbox style) ke user. " +
    ...
    // ── 3. ui_confirm ─────────────────────────────────────────────────────────
    server.tool("ui_confirm", "Tampilkan dialog konfirmasi sebelum AI melakukan aksi yang tidak bisa di-undo. " +
    ...
    // ── 4. ui_menu ────────────────────────────────────────────────────────────
    server.tool("ui_menu", "Tampilkan menu navigasi berjenjang. Berguna untuk mengekspos fitur-fitur " +
    ...
    // ── 5. ui_progress ────────────────────────────────────────────────────────
    server.tool("ui_progress", "Tampilkan progress tracker untuk task multi-step yang panjang. " +
    ...
    // ── 6. ui_info_card ───────────────────────────────────────────────────────
    server.tool("ui_info_card", "Tampilkan kartu informasi terstruktur dengan key-value pairs. " +
    ...
    // ── 7. ui_input_form ──────────────────────────────────────────────────────
    server.tool("ui_input_form", "Tampilkan form terstruktur untuk mengumpulkan beberapa input sekaligus " +
    ...
    // ── 8. ui_table ───────────────────────────────────────────────────────────
    server.tool("ui_table", "Render data dalam bentuk tabel ASCII yang rapi. " +
        ...
        try {
            ...
            const renderRow = (cells, isHeader = false, highlight = false) => {
                const sep = isHeader ? "│" : "│";
            ...
            const borderLine = (left, mid, right) => left + colWidths.map((w) => "─".repeat(w + 2)).join(mid) + right;
            const lines = [
                `📊 ${title}`,
                borderLine("┌", "┬", "┐"),
                renderRow(headers, true),
                borderLine("├", "┼", "┤"),
                ...rows.map((row, i) => renderRow(headers.map((_, ci) => row[ci] ?? ""), false, i === highlight_row)),
                borderLine("└", "┴", "┘"),
            ];
            if (footer)
                lines.push(`\n📌 ${footer}`);
    ...
    // ── 9. ui_notification ───────────────────────────────────────────────────
    server.tool("ui_notification", "Tampilkan notifikasi/alert bergaya ke user. " +
