 ...
 */
async function evictOldestIdleSession() {
    if (sessions.size === 0)
        return null;
    ...
    process.stderr.write(`[browser] Session limit reached (${MAX_ACTIVE_SESSIONS}). ` +
...
async function getActivePage(sessionId) {
...
export function registerBrowserTools(server) {
    ...
    server.tool("browser_open", "Membuka browser baru dan navigasi ke URL. Membuat 'session' yang bisa " +
        ...
        session_id: z
            .string()
            .optional()
            .describe("ID session yang sudah ada (untuk reuse browser). " +
    ...
    }, async ({ url, session_id, device, stealth, wait_until, timeout_seconds, take_screenshot: doScreenshot, }) => {
        try {
            ...
            if (session_id && sessions.has(session_id)) {
                ...
                if (sessions.size >= MAX_ACTIVE_SESSIONS) {
                    ...
                    process.stderr.write(`[browser] Auto-evicted session ${evictedId ?? "?"} to make room.\n`);
                }
                // Buat session baru
                const extraArgs = [
            ...
            if (!result) {
                return {
                    content: [
                        {
                            type: "text",
                            text: `❌ Session '${session_id}' tidak ditemukan atau sudah expired.\n` +
    ...
    server.tool("browser_click", "Klik elemen di halaman menggunakan CSS selector. " +
        ...
        selector: z
            .string()
            .describe("CSS selector elemen yang diklik. Contoh: '#submit-btn', " +
    ...
    // ── 4. browser_type ───────────────────────────────────────────────────────
    server.tool("browser_type", "Ketik teks ke dalam input field, textarea, atau elemen yang bisa di-edit. " +
    ...
    }, async ({ session_id, selector, text, clear_first, press_enter, delay_ms, take_screenshot: doScreenshot, }) => {
        try {
            const result = await getActivePage(session_id);
            if (!result) {
                return {
    ...
    // ── 6. browser_scroll ─────────────────────────────────────────────────────
    server.tool("browser_scroll", "Scroll halaman ke posisi tertentu, scroll ke elemen, atau scroll ke bawah/atas.", {
        session_id: z.string().describe("Session ID"),
        direction: z
        ...
        try {
            ...
            switch (direction) {
                case "up":
                    await page.evaluate((px) => window.scrollBy(0, -px), amount_px ?? 500);
                    break;
                case "down":
                    await page.evaluate((px) => window.scrollBy(0, px), amount_px ?? 500);
                    break;
                ...
                case "to_element":
                    if (!selector)
                        throw new Error("selector wajib untuk direction 'to_element'");
                    await page.evaluate((sel) => {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `✅ Scroll '${direction}' berhasil!\n` +
                            `${"─".repeat(50)}\n` +
    ...
    server.tool("browser_get_content", "Ambil konten halaman saat ini: HTML, teks, atau data spesifik dari elemen. " +
        ...
        content_type: z
            ...
            .default("text")
            .describe("Jenis konten: html=full HTML, text=teks saja, element_text=teks dari selector, " +
    ...
    // ── 8. browser_wait ───────────────────────────────────────────────────────
    server.tool("browser_wait", "Tunggu kondisi tertentu sebelum melanjutkan: " +
        ...
        try {
            ...
            switch (wait_type) {
                case "selector_visible":
                    if (!selector)
                        throw new Error("selector wajib");
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `⏳ Wait '${wait_type}' selesai (${elapsed}ms)\n` +
                            `${"─".repeat(50)}\n` +
                            pageInfo,
                    },
                ],
    ...
    // ── 9. browser_select ─────────────────────────────────────────────────────
    server.tool("browser_select", "Pilih opsi dari dropdown <select>, atau toggle checkbox/radio button.", {
        session_id: z.string().describe("Session ID"),
        ...
        try {
            ...
            switch (action) {
                case "select_option": {
                    ...
                    if (!selectValue)
                        throw new Error("value atau label wajib untuk select_option");
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `✅ Form action '${action}' berhasil!\n` +
                            `${"─".repeat(50)}\n` +
                            `🎯 Selector : ${selector}\n` +
                            `📝 Hasil    : ${resultInfo}` +
                            ssInfo,
    ...
    server.tool("browser_execute_script", "Eksekusi JavaScript di browser session aktif dan kembalikan hasilnya. " +
        ...
        script: z
            .string()
            .describe("JavaScript yang dieksekusi di context halaman. " +
    ...
    // ── 11. browser_close ─────────────────────────────────────────────────────
    server.tool("browser_close", "Tutup browser session dan bebaskan memori. " +
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `✅ Browser session '${session_id}' ditutup.\n` +
    ...
    server.tool("browser_list_sessions", "List semua browser session yang sedang aktif beserta info URL dan waktu.", {}, async () => {
        try {
            if (sessions.size === 0) {
                return {
                    content: [
    ...
    // ── 13. browser_hover ─────────────────────────────────────────────────────
    server.tool("browser_hover", "Hover mouse di atas elemen untuk memicu tooltip, dropdown menu, atau efek hover CSS.", {
        session_id: z.string().describe("Session ID"),
    ...
    // ── 14. browser_keyboard ──────────────────────────────────────────────────
    server.tool("browser_keyboard", "Tekan tombol keyboard khusus: Enter, Tab, Escape, Arrow keys, Ctrl+C, dll. " +
        "Berguna untuk navigasi form, menutup modal, shortcut keyboard.", {
        session_id: z.string().describe("Session ID"),
        keys: z
            .array(z.string())
            .describe("Tombol yang ditekan. Contoh: ['Enter'], ['Tab'], ['Escape'], " +
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `⌨️  Keys '${keys.join("+")}' ditekan!` + ssInfo,
                    },
                ],
            };
        }
