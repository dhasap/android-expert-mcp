...
export function registerVpsTools(server) {
    ...
    // ── 5. vps_deploy ────────────────────────────────────────────────────────
    server.tool("vps_deploy", "Deploy project ke VPS via SCP + SSH. " +
        ...
        try {
            ...
            await runCommand(buildSshCmd(profile, `mkdir -p "${remote_path}"`), undefined, 15000);
            ...
            if (post_commands.length > 0 && deployStatus === "success") {
                ...
                for (const cmd of post_commands) {
                    lines.push(`   $ ${cmd}`);
                    const r = await runCommand(buildSshCmd(profile, `cd "${remote_path}" && ${cmd}`), undefined, 120000);
            ...
            if (manager === "pm2") {
                switch (action) {
                    case "list":
                        ...
                        break;
                    case "status":
                        cmd = service_name ? `pm2 show ${service_name}` : "pm2 list";
                        break;
                    case "start":
                        cmd = `pm2 start ${service_name}`;
                        break;
                    case "stop":
                    ...
                    case "restart":
                        ...
                        break;
                    case "reload":
                        cmd = `pm2 reload ${service_name}`;
                        break;
                    case "logs":
                        cmd = `pm2 logs ${service_name ?? ""} --lines 30 --nostream`;
                        break;
                    case "save":
                        cmd = "pm2 save";
                        break;
                ...
                switch (action) {
                    ...
                    case "reload":
                        ...
                        break;
                    case "logs":
                        cmd = `journalctl -u ${service_name} -n 30 --no-pager`;
                        break;
                    case "save":
                        cmd = `systemctl enable ${service_name}`;
                        break;
                    case "startup":
                        cmd = `systemctl enable ${service_name} && systemctl start ${service_name}`;
                        break;
                }
            }
