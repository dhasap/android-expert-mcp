...
import { atomicReadJson, atomicWriteJson, ensureDir, formatToolError, truncateOutput, Mutex, } from "../utils.js";
// ─── Constants ────────────────────────────────────────────────────────────────
...
async function withSnapshots(name, fn) {
    const release = await SNAPSHOT_MUTEX.acquire();
    try {
        const filePath = await getSnapshotPath(name);
        const index = await atomicReadJson(filePath, { snapshots: [] });
...
function compactBuildLog(text) {
    ...
    for (const raw of lines) {
        ...
        if (/^\s+at [\w.$]+/.test(line) || /^Caused by:/.test(line)) {
            if (kept.length < 150)
                kept.push(line); // limit stack trace depth
            inStackTrace = true;
            continue;
        }
        inStackTrace = false;
        ...
        if (/^BUILD (SUCCESS|FAILED)/.test(line)) {
            kept.push(line);
            continue;
        }
    ...
    return (`[BUILD LOG — compacted ${saved}% | ${original}→${compacted} tokens]\n` +
        `${"─".repeat(60)}\n` +
        deduped.join("\n"));
}
/**
 ...
 */
function compactAdbLog(text, packageFilter) {
    const lines = text.split("\n");
    const kept = [];
    let inCrash = false;
    for (const raw of lines) {
    ...
    const saved = Math.round((1 - compacted / original) * 100);
    return (`[ADB LOG — compacted ${saved}% | ${original}→${compacted} tokens]\n` +
        `${"─".repeat(60)}\n` +
        kept.join("\n"));
}
/**
 ...
 */
function compactServerLog(text) {
    const lines = text.split("\n");
    const kept = [];
    for (const raw of lines) {
        const line = raw.trimEnd();
    ...
    const compacted = estimateTokens(kept.join("\n"));
    const saved = Math.round((1 - compacted / original) * 100);
    return (`[SERVER LOG — compacted ${saved}% | ${original}→${compacted} tokens]\n` +
        `${"─".repeat(60)}\n` +
        kept.join("\n"));
}
/**
 ...
 */
function compactCode(text, lang) {
    const lines = text.split("\n");
    const kept = [];
    let blankRun = 0;
    for (const raw of lines) {
    ...
    const compacted = estimateTokens(kept.join("\n"));
    const saved = Math.round((1 - compacted / original) * 100);
    return (`[CODE${lang ? ` (${lang})` : ""} — compacted ${saved}% | ${original}→${compacted} tokens]\n` +
        `${"─".repeat(60)}\n` +
        kept.join("\n"));
}
/**
...
function compactGeneric(text) {
    ...
    const compacted = estimateTokens(kept.join("\n"));
    const saved = Math.round((1 - compacted / original) * 100);
    return (`[GENERIC — compacted ${saved}% | ${original}→${compacted} tokens]\n` +
        `${"─".repeat(60)}\n` +
        kept.join("\n"));
}
// ─── Tool registration ────────────────────────────────────────────────────────
export function registerContextManagerTools(server) {
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `✅ Context snapshot disimpan!\n` +
                            `${"─".repeat(60)}\n` +
                            `ID      : ${newSnapshot.id}\n` +
                            `Project : ${project}\n` +
                            `Task    : ${current_task}\n` +
    ...
    // ── 2. context_load ───────────────────────────────────────────────────────
    server.tool("context_load", "Muat snapshot konteks sesi yang tersimpan. " +
        ...
        project: z
            .string()
            .describe("Nama proyek. Harus sama dengan nama yang dipakai saat context_save. " +
            "Gunakan context_list() jika tidak ingat nama proyeknya."),
        snapshot_id: z
            .string()
            .optional()
            .describe("ID snapshot spesifik (opsional). Kosongkan untuk load snapshot TERBARU. " +
        ...
        try {
            ...
            if (index.snapshots.length === 0) {
                return {
                    content: [
                        {
                            type: "text",
                            text: `❌ Tidak ada snapshot untuk project "${project}".\n\n` +
            ...
            const age = Math.round((Date.now() - new Date(snap.updated_at).getTime()) / (1000 * 60));
            const ageStr = age < 60
                ? `${age} menit lalu`
                : age < 1440
            ...
            if (index.snapshots.length > 1) {
                lines.push("", `💡 Ada ${index.snapshots.length - 1} snapshot lama lainnya — lihat dengan context_list(project="${project}")`);
            }
    ...
    // ── 3. context_list ───────────────────────────────────────────────────────
    server.tool("context_list", "Tampilkan semua project yang memiliki snapshot tersimpan, " +
        "atau tampilkan riwayat snapshot dalam satu project.", {
        project: z
            .string()
            .optional()
            .describe("Nama project untuk tampilkan riwayat snapshots-nya. " +
        ...
        try {
            ...
            if (project) {
                ...
                for (const s of index.snapshots) {
                    const age = Math.round((Date.now() - new Date(s.updated_at).getTime()) / (1000 * 60 * 60));
            ...
            for (const file of jsonFiles) {
                ...
                const index = await atomicReadJson(path.join(SNAPSHOT_DIR, file), { snapshots: [] });
                if (index.snapshots.length === 0)
                    continue;
                const latest = index.snapshots[0];
                const age = Math.round((Date.now() - new Date(latest.updated_at).getTime()) / (1000 * 60 * 60));
                const ageStr = age < 24 ? `${age}j lalu` : `${Math.round(age / 24)}hr lalu`;
                lines.push(`📁 ${projName}  (${index.snapshots.length} snapshot, terakhir ${ageStr})`);
    ...
    // ── 4. context_delete ─────────────────────────────────────────────────────
    server.tool("context_delete", "Hapus snapshot tertentu atau semua snapshot dalam sebuah project.", {
        project: z.string().describe("Nama project"),
        snapshot_id: z
            .string()
            .optional()
            .describe("ID snapshot yang akan dihapus. Kosongkan untuk hapus SEMUA snapshot project ini."),
    }, async ({ project, snapshot_id }) => {
        try {
            if (!snapshot_id) {
    ...
    // ── 5. context_compact ────────────────────────────────────────────────────
    server.tool("context_compact", "Padatkan teks panjang (log, output Gradle, kode, dsb.) menjadi versi yang jauh lebih ringkas " +
    ...
    server.tool("context_compact_file", "Baca file dari disk dan langsung padatkan isinya. " +
        ...
        try {
            ...
            const header = `📄 File: ${file_path} (${fileSizeKb.toFixed(1)} KB)\n` +
                `Mode: ${mode}\n` +
                `${"─".repeat(60)}\n`;
            return {
    ...
    // ── 7. context_stats ──────────────────────────────────────────────────────
    server.tool("context_stats", "Tampilkan statistik penggunaan Context Manager: " +
        "jumlah project tersimpan, total snapshots, ukuran storage, " +
        "dan tips untuk menghemat context window.", {}, async () => {
        try {
            await ensureDir(SNAPSHOT_DIR);
            let files = [];
            try {
            ...
            const storageSizeStr = totalSizeBytes < 1024
                ? `${totalSizeBytes} B`
            ...
            lines.push("💡 Tips hemat context window:", "  1. Panggil context_save() sebelum sesi panjang berakhir", "  2. Di sesi baru, mulai dengan context_load() — hemat puluhan pesan penjelasan ulang", "  3. Pakai context_compact(mode='build_log') untuk log Gradle — hemat 70-90% tokens", "  4. Pakai context_compact(mode='code') sebelum paste file .kt/.ts besar", "  5. Gunakan context_compact_file() untuk file log > 100KB langsung dari path", "  6. Simpan error penting ke error_remember() — bisa dicari di sesi lain tanpa paste ulang");
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (error) {
