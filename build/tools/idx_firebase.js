...
export function registerIdxFirebaseTools(server) {
    ...
    }, async ({ device_serial, action, x, y, x2, y2, swipe_duration_ms, text, resource_id, take_screenshot: doScreenshot, }) => {
        try {
            const devices = await getConnectedDevicesList();
            if (devices.length === 0) {
                return {
            ...
            if (action === "tap_by_text" || action === "tap_by_id") {
                ...
                await runAdbCommand(`adb ${flag} shell uiautomator dump --compressed ${remotePath}`, undefined, 20000);
                const localTmp = path.join(os.tmpdir(), `ui_tmp_${Date.now()}.xml`);
                await runAdbCommand(`adb ${flag} pull ${remotePath} "${localTmp}"`, undefined, 10000);
                await runAdbCommand(`adb ${flag} shell rm ${remotePath}`, undefined, 5000);
                const xmlContent = await fs.readFile(localTmp, "utf-8").catch(() => "");
                // Cari elemen berdasarkan text atau resource-id
                const searchAttr = action === "tap_by_text"
                    ? `text="${text}"`
                    : `resource-id="${resource_id}"`;
                const nodePattern = new RegExp(`<node[^>]*${searchAttr.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[^>]*bounds="\\[(\\d+),(\\d+)\\]\\[(\\d+),(\\d+)\\]"`);
                ...
                if (!match) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: `❌ Elemen tidak ditemukan: ${searchAttr}\n` +
    ...
    server.tool("emulator_input_text", "Ketik teks ke field yang sedang fokus di emulator. " +
        ...
        take_screenshot: z.boolean().default(true),
    }, async ({ device_serial, text, tap_x, tap_y, resource_id, clear_first, press_enter_after, take_screenshot: doScreenshot, }) => {
        try {
            const devices = await getConnectedDevicesList();
            if (devices.length === 0) {
                return {
            ...
            if (tap_x !== undefined && tap_y !== undefined) {
                await runAdbCommand(`adb ${flag} shell input tap ${tap_x} ${tap_y}`, undefined, 5000);
                ...
                const match = xmlContent.match(new RegExp(`<node[^>]*resource-id="${escapedId}"[^>]*bounds="\\[(\\d+),(\\d+)\\]\\[(\\d+),(\\d+)\\]"`));
                if (match) {
                    const cx = Math.round((parseInt(match[1]) + parseInt(match[3])) / 2);
                    const cy = Math.round((parseInt(match[2]) + parseInt(match[4])) / 2);
                    await runAdbCommand(`adb ${flag} shell input tap ${cx} ${cy}`, undefined, 5000);
                    await new Promise((r) => setTimeout(r, 300));
                }
            }
            if (clear_first) {
                // Ctrl+A lalu Delete
                await runAdbCommand(`adb ${flag} shell input keyevent KEYCODE_CTRL_A`, undefined, 3000);
                await runAdbCommand(`adb ${flag} shell input keyevent KEYCODE_DEL`, undefined, 3000);
            }
            // Escape spasi dan karakter khusus untuk ADB input text
            const escapedText = text.replace(/ /g, "%s").replace(/&/g, "\\&");
            await runAdbCommand(`adb ${flag} shell input text "${escapedText}"`, undefined, 10000);
