...
import { runCommand, runAdbCommand, formatToolError, ensureDir, truncateOutput, extractStackTrace, shellEscape, } from "../utils.js";
// ─── Konstanta IDX ────────────────────────────────────────────────────────────
...
async function waitForUiIdle(deviceSerial, timeoutMs = 5000) {
    const flag = deviceSerial ? `-s ${deviceSerial}` : "";
    const start = Date.now();
    // Try using window animation scale check
    while (Date.now() - start < timeoutMs) {
        const result = await runAdbCommand(`adb ${flag} shell dumpsys window animations | grep -E "AnimationState|mAnimating" | head -5`, undefined, 3000);
...
async function findElementInUi(deviceSerial, searchType, searchValue) {
    const flag = deviceSerial ? `-s ${deviceSerial}` : "";
    const remotePath = `/sdcard/ui_find_${Date.now()}.xml`;
    try {
        // Dump UI with retry
        await withRetry(async () => {
            const dumpResult = await runAdbCommand(`adb ${flag} shell uiautomator dump --compressed ${remotePath}`, undefined, 15000);
            if (dumpResult.exitCode !== 0)
                throw new Error("UI dump failed");
        }, 3, 300);
        const localPath = path.join(os.tmpdir(), `ui_find_${Date.now()}.xml`);
        await runAdbCommand(`adb ${flag} pull ${remotePath} "${localPath}"`, undefined, 5000);
        ...
        const nodePattern = new RegExp(`<node[^>]*${searchAttr.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}[^>]*clickable="([^"]*)"[^>]*bounds="\\[(\\d+),(\\d+)\\]\\[(\\d+),(\\d+)\\]"`);
...
async function verifyElementAtCoords(deviceSerial, x, y, expectedText) {
    if (!expectedText)
        return true; // Skip verification if no text to match
    const flag = deviceSerial ? `-s ${deviceSerial}` : "";
    const remotePath = `/sdcard/ui_verify_${Date.now()}.xml`;
    try {
        await runAdbCommand(`adb ${flag} shell uiautomator dump --compressed ${remotePath}`, undefined, 10000);
        ...
        const pattern = new RegExp(`<node[^>]*text="${escapedText}"[^>]*bounds="\\[(\\d+),(\\d+)\\]\\[(\\d+),(\\d+)\\]"`);
        ...
        const tolerance = 50;
        return (Math.abs(centerX - x) <= tolerance &&
...
async function smartTap(deviceSerial, x, y, expectedText, maxRetries = 3) {
    const flag = deviceSerial ? `-s ${deviceSerial}` : "";
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            // Execute tap
            const result = await runAdbCommand(`adb ${flag} shell input tap ${x} ${y}`, undefined, 5000);
...
async function smartSwipe(deviceSerial, x1, y1, x2, y2, durationMs) {
    const flag = deviceSerial ? `-s ${deviceSerial}` : "";
    ...
    try {
        ...
        const result = await runAdbCommand(`adb ${flag} shell input swipe ${x1} ${y1} ${x2} ${y2} ${duration}`, undefined, 5000);
        if (result.exitCode === 0) {
...
async function smartInputText(deviceSerial, text, options) {
    const flag = deviceSerial ? `-s ${deviceSerial}` : "";
    try {
        // Tap to focus field if coordinates provided
        if (options.tapX !== undefined && options.tapY !== undefined) {
            await waitForUiIdle(deviceSerial, 1000);
            const tapResult = await runAdbCommand(`adb ${flag} shell input tap ${options.tapX} ${options.tapY}`, undefined, 5000);
        ...
        for (const chunk of chunks) {
            ...
            await withRetry(async () => {
                const result = await runAdbCommand(`adb ${flag} shell input text "${escaped}"`, undefined, 5000);
                if (result.exitCode !== 0)
        ...
        if (options.pressEnter) {
            await delay(100);
            await runAdbCommand(`adb ${flag} shell input keyevent KEYCODE_ENTER`, undefined, 3000);
        }
        await delay(200);
...
async function getConnectedDevicesList() {
...
async function waitForTestLabOperation(operationId, maxWaitSeconds, projectId) {
    ...
    while (Date.now() < deadline) {
        ...
        const r = await runCommand(`gcloud firebase test android operations describe "${shellEscape(operationId)}" --project "${shellEscape(projectId)}" --format=json`, undefined, 15000);
        if (r.exitCode === 0) {
            try {
...
// ─── Tool Registration ────────────────────────────────────────────────────────
export function registerIdxFirebaseTools(server) {
    // ════════════════════════════════════════════════════════════════════════════
    // BAGIAN A — IDX EMULATOR CONNECTION
    // ════════════════════════════════════════════════════════════════════════════
    // ── A1. idx_detect_emulator ───────────────────────────────────────────────
    server.tool("idx_detect_emulator", "Deteksi otomatis emulator Android yang berjalan di environment IDX Firebase Studio. " +
        "Scan port ADB standar (5554–5562) dan coba koneksi TCP/IP. " +
        "Gunakan ini PERTAMA KALI saat physical device tidak tersedia.", {
        host: z
            .string()
            .default("localhost")
            .describe("Host emulator. Di IDX: 'localhost' atau '127.0.0.1'. " +
        ...
        try {
            ...
            for (const port of ports) {
                ...
                const pingResult = await runCommand(`nc -zv -w2 "${safeHost}" ${port} 2>&1 || echo "CLOSED"`, undefined, 5000);
                const isOpen = pingResult.stdout.includes("open") ||
            ...
            if (auto_connect && found.length > 0) {
                ...
                for (const f of found) {
                    const connectResult = await runAdbCommand(`adb connect ${f.address}`, undefined, 10000);
    ...
    server.tool("idx_connect_emulator", "Koneksikan ADB ke emulator via TCP/IP (tanpa USB). " +
        ...
        try {
            ...
            if (wait_for_boot) {
                ...
                while (Date.now() - bootStart < boot_timeout_seconds * 1000) {
                    const bootCheck = await runAdbCommand(`adb -s ${address} shell getprop sys.boot_completed`, undefined, 10000);
            ...
            // Info device
            const propResult = await runAdbCommand(`adb -s ${address} shell getprop ro.product.model`, undefined, 5000);
            const sdkResult = await runAdbCommand(`adb -s ${address} shell getprop ro.build.version.sdk`, undefined, 5000);
            const androidResult = await runAdbCommand(`adb -s ${address} shell getprop ro.build.version.release`, undefined, 5000);
            lines.push("");
            lines.push("📱 Info Emulator:");
            lines.push(`   Model   : ${propResult.stdout.trim() || "unknown"}`);
            lines.push(`   Android : ${androidResult.stdout.trim() || "unknown"} (API ${sdkResult.stdout.trim() || "?"})`);
    ...
    server.tool("idx_emulator_status", "Cek status lengkap emulator yang terkoneksi: " +
        ...
        try {
            ...
            if (devices.length === 0) {
                return {
                    content: [
                        {
                            type: "text",
                            text: "❌ Tidak ada device terkoneksi.\n" +
            ...
            const [model, android, sdk, mem, cpu, storage, uptime, display] = await Promise.all([
                ...
                runAdbCommand(`adb ${flag} shell dumpsys display | grep "mCurrentDisplayRect\\|DisplayWidth\\|DisplayHeight" | head -3`, undefined, 5000),
            ]);
            // Parse memory
            const memLines = mem.stdout.split("\n");
            const memTotal = memLines.find((l) => l.startsWith("MemTotal"))?.replace(/\s+/g, " ") ?? "N/A";
            const memAvail = memLines.find((l) => l.startsWith("MemAvailable"))?.replace(/\s+/g, " ") ?? "N/A";
            const lines = [
                "📱 Emulator Status",
                "═".repeat(55),
    ...
    // ── A4. idx_install_apk ───────────────────────────────────────────────────
    server.tool("idx_install_apk", "Install APK ke emulator IDX. Otomatis mendeteksi emulator yang " +
    ...
    }, async ({ apk_path, device_serial, replace_existing, grant_permissions, launch_after, package_name, }) => {
        try {
            ...
            const installResult = await runAdbCommand(`adb ${flag} install ${flags} "${resolvedApk}"`, undefined, 120000);
            if (installResult.exitCode !== 0 ||
                installResult.stdout.includes("Failure") ||
                installResult.stderr.includes("Failure")) {
                const errMsg = installResult.stdout + "\n" + installResult.stderr;
                lines.push("");
                ...
                if (launch_after && package_name) {
                    ...
                    if (!/^[a-z][a-z0-9_]*(\.[a-z][a-z0-9_]*)+$/.test(package_name)) {
                        ...
                        const launchResult = await runAdbCommand(`adb ${flag} shell monkey -p "${safePackage}" -c android.intent.category.LAUNCHER 1`, undefined, 15000);
                        if (launchResult.exitCode === 0) {
                            lines.push(`✅ Aplikasi ${package_name} dilaunched.`);
                        }
                        else {
                            lines.push(`⚠️  Launch gagal: ${launchResult.stderr.trim()}`);
                        }
                    }
                }
            }
    ...
    // ════════════════════════════════════════════════════════════════════════════
    // ── B1. ftl_run_test ──────────────────────────────────────────────────────
    server.tool("ftl_run_test", "Upload APK dan jalankan test di Firebase Test Lab. Support Robo Test dan Instrumentation Test." +
        ...
        test_type: z
            ...
            .default("robo")
            .describe("Tipe test: 'robo' = otomatis tanpa kode test, " +
        ...
        device_model: z
            .string()
            .default("Pixel6")
            .describe("Model device Test Lab. Gunakan ftl_list_devices untuk daftar lengkap. " +
        ...
        robo_directives: z
            .string()
            .optional()
            .describe("Robo directives untuk isi form otomatis. Format: 'field_resource_id:value'. " +
    ...
    }, async ({ project_id, apk_path, test_type, test_apk_path, device_model, android_version, locale, orientation, timeout_minutes, robo_directives, results_bucket, wait_for_results, results_dir, }) => {
        try {
            if (!(await isGcloudAvailable())) {
                return {
                    content: [
                        {
                            type: "text",
                            text: "❌ gcloud CLI tidak tersedia.\n\n" +
            ...
            const testResult = await runCommand(gcloudArgs.join(" "), undefined, (timeout_minutes + 5) * 60 * 1000);
            ...
            const testState = testData["@type"]?.includes("TestMatrix")
                ? "submitted"
                : "unknown";
            lines.push(`   Status  : ${testState}`);
            if (testResult.stdout.includes("Passed"))
                lines.push("   ✅ Test PASSED");
            if (testResult.stdout.includes("Failed"))
                lines.push("   ❌ Test FAILED");
            if (testResult.stdout.includes("Skipped"))
                lines.push("   ⏭️  Test SKIPPED");
            // Extract GCS results URL
            const gcsMatch = testResult.stdout.match(/gs:\/\/[^\s]+/);
            const webMatch = testResult.stdout.match(/https:\/\/console\.firebase\.google\.com[^\s]+/);
            if (gcsMatch) {
    ...
    // ── B2. ftl_list_devices ──────────────────────────────────────────────────
    server.tool("ftl_list_devices", "Tampilkan daftar device model yang tersedia di Firebase Test Lab " +
        ...
        try {
            ...
            const result = await runCommand(`gcloud firebase test android models list --project "${shellEscape(project_id)}" --format="table(id,name,supportedVersionIds,tags)"`, undefined, 30000);
            if (result.exitCode !== 0) {
                return {
                    content: [
                        {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: "📱 Firebase Test Lab — Available Devices\n" +
                            "─".repeat(55) +
                            "\n" +
                            output +
    ...
    server.tool("ftl_download_results", "Download hasil test dari Firebase Test Lab: screenshot, video, logcat, " +
        "dan test report XML dari Google Cloud Storage.", {
        gcs_path: z
            .string()
            .describe("Path GCS hasil test. Format: 'gs://bucket-name/path/to/results'. " +
        ...
        try {
            ...
            for (const [type, pattern] of Object.entries(filePatterns)) {
                if (!downloadAll && !download_types.includes(type)) {
                    continue;
                }
                ...
                const dlResult = await runCommand(`gsutil -m cp -r "${gcsSource}" "${typeDir}/" 2>&1`, undefined, 120000);
                if (dlResult.exitCode === 0 || dlResult.stdout.includes("Copying")) {
                    const files = await fs.readdir(typeDir).catch(() => []);
                    lines.push(`   ✅ ${files.length} file(s) didownload ke ${typeDir}`);
            ...
            lines.push("");
            lines.push(`✅ Download selesai. Total: ${downloadedFiles.length} file(s)`);
    ...
    // ── B4. ftl_parse_report ──────────────────────────────────────────────────
    server.tool("ftl_parse_report", "Parse dan tampilkan hasil test report dari Firebase Test Lab. " +
        "Mendukung format XML (JUnit) dan JSON. " +
        "Memberikan ringkasan: passed/failed/skipped, waktu eksekusi, error messages.", {
        report_path: z
            .string()
            .describe("Path ke file report. Bisa XML (test_result_*.xml) atau JSON."),
        ...
        try {
            ...
            if (ext === ".xml") {
                // Parse JUnit XML
                const suiteMatch = content.match(/<testsuite[^>]*name="([^"]*)"[^>]*tests="(\d+)"[^>]*failures="(\d+)"[^>]*errors="(\d+)"[^>]*skipped="(\d+)"[^>]*time="([^"]*)"/);
                if (suiteMatch) {
                    const [, name, tests, failures, errors, skipped, time] = suiteMatch;
                ...
                const testCasePattern = /<testcase[^>]*name="([^"]*)"[^>]*classname="([^"]*)"[^>]*time="([^"]*)"[^>]*(?:\/>|>([\s\S]*?)<\/testcase>)/g;
                let match;
                const testCases = [];
                let testCount = 0;
                ...
                if (toShow.length > 0) {
                    ...
                    toShow.forEach((tc) => {
                        ...
                        if (tc.message) {
                            lines.push(`     ↳ ${tc.message.slice(0, 200).replace(/\n/g, " ")}`);
                        }
                    });
                }
            }
    ...
    // ── C2. emulator_ui_dump ──────────────────────────────────────────────────
    server.tool("emulator_ui_dump", "Dump hierarki UI dari emulator via uiautomator dan parse menjadi " +
        ...
        try {
            ...
            // Pull XML
            await runAdbCommand(`adb ${flag} pull ${remotePath} "${localPath}"`, undefined, 15000);
            await runAdbCommand(`adb ${flag} shell rm ${remotePath}`, undefined, 5000);
            ...
            const MAX_NODES = 5000; // Safety limit to prevent OOM/hang
            while ((match = nodePattern.exec(xmlContent)) !== null && nodeCount < MAX_NODES) {
                const [, cls, resourceId, text, contentDesc, , , clickable, enabled, , , scrollable, x1, y1, x2, y2,] = match;
                // Filter by package if specified
                if (package_filter && resourceId && !resourceId.startsWith(package_filter)) {
                    continue;
                }
                nodeCount++;
                const isInput = (cls?.includes("EditText") ||
                    cls?.includes("Input") ||
            ...
            if (parse_mode === "text_only") {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: "📝 Teks yang terlihat di layar:\n" +
                                "─".repeat(55) +
                                "\n" +
                                texts.join("\n"),
                        },
            ...
            if (parse_mode === "interactive_only" || parse_mode === "summary") {
                if (interactive.length > 0) {
                    ...
                    interactive.slice(0, 50).forEach((n) => {
                        ...
                        lines.push(`  ${typeIcon} [${n.class}] "${label.slice(0, 60)}"` +
                            (n.resourceId ? `\n     id: ${n.resourceId}` : "") +
                            `\n     bounds: ${n.bounds}`);
                    });
                }
    ...
    server.tool("emulator_tap", "Tap/klik elemen UI di emulator via ADB. Support tap by koordinat, text, atau resource-id.", {
        ...
        swipe_duration_ms: z.number().int().optional().describe("Durasi swipe (ms). Auto-calculate jika tidak diisi."),
        ...
        verify_tap: z.boolean().default(true).describe("Verifikasi elemen masih ada sebelum tap (untuk tap_by_text)"),
        retry_count: z.number().int().min(1).max(5).default(3).describe("Jumlah retry jika tap gagal (default: 3)"),
    }, async ({ device_serial, action, x, y, x2, y2, swipe_duration_ms, text, resource_id, take_screenshot: doScreenshot, verify_tap, retry_count, }) => {
        try {
            const devices = await getConnectedDevicesList();
            if (devices.length === 0) {
                return {
                ...
                if (!elem) {
                    return {
                        content: [
                            {
                                type: "text",
                                text: `❌ Elemen tidak ditemukan: ${searchType}="${searchValue}"\n` +
            ...
            switch (action) {
                ...
                case "tap_by_id":
                    ...
                    result = await smartTap(device_serial, tapX, tapY, verify_tap ? targetText : undefined, retry_count);
                    break;
                case "long_press":
                    if (tapX === undefined || tapY === undefined) {
                        return {
    ...
    server.tool("emulator_input_text", "Ketik teks ke field yang sedang fokus di emulator. " +
        ...
        take_screenshot: z.boolean().default(true),
        verify_field_focused: z.boolean().default(true).describe("Verifikasi field fokus sebelum typing"),
    }, async ({ device_serial, text, tap_x, tap_y, resource_id, clear_first, press_enter_after, take_screenshot: doScreenshot, verify_field_focused, }) => {
        try {
            const devices = await getConnectedDevicesList();
            if (devices.length === 0) {
                return {
    ...
    server.tool("emulator_record_screen", "Rekam video layar emulator (screenrecord). " +
        ...
        try {
            ...
            const localVideo = output_path ?? path.join(vidDir, `record_${timestamp}.mp4`);
            // Mulai rekaman (blocking)
            const recordResult = await runAdbCommand(`adb ${flag} shell screenrecord --time-limit ${duration_seconds} --bit-rate ${bit_rate_mbps}000000 ${remoteVideo}`, undefined, (duration_seconds + 10) * 1000);
            // Pull video
            await runAdbCommand(`adb ${flag} pull ${remoteVideo} "${localVideo}"`, undefined, 30000);
            await runAdbCommand(`adb ${flag} shell rm ${remoteVideo}`, undefined, 5000);
            let stat;
            try {
...
//# sourceMappingURL=idx_firebase.js.map