...
// ═══════════════════════════════════════════════════════════════════════════
async function makeHttpRequest(url, method, headers, body, timeoutMs = 30000) {
...
export function registerAdvancedTestingTools(server) {
    ...
    // ── 2. api_mock_server ──────────────────────────────────────────────────
    server.tool("api_mock_server", "Start/stop mock HTTP server untuk testing.", {
        ...
        try {
            const lines = [`🎭 Mock Server — ${action.toUpperCase()}`];
            switch (action) {
                case "start": {
    ...
    // ── 3. profile_memory ───────────────────────────────────────────────────
    server.tool("profile_memory", "Analisis memory usage aplikasi Android. Deteksi memory leaks.", {
        ...
        try {
            ...
            while (Date.now() - startTime < duration_seconds * 1000) {
                const memResult = await runAdbCommand(`adb ${flag} shell dumpsys meminfo ${package_name} | grep -E "TOTAL PSS|TOTAL PRIVATE|Java Heap:"`, undefined, 5000);
            ...
            if (samples.length > 1) {
                ...
                if (pssGrowth > 10)
                    lines.push("   ⚠️  WARNING: Possible memory leak!");
            }
            return { content: [{ type: "text", text: lines.join("\n") }] };
        }
        catch (error) {
    ...
    // ── 4. profile_cpu ──────────────────────────────────────────────────────
    server.tool("profile_cpu", "CPU profiling untuk aplikasi Android.", {
        device_serial: z.string().optional(),
        package_name: z.string(),
        ...
        try {
            ...
            const topResult = await runAdbCommand(`adb ${flag} shell "top -p $(pidof ${package_name}) -b -n ${Math.min(duration_seconds, 10)} 2>/dev/null | head -30"`, undefined, (duration_seconds + 5) * 1000);
    ...
    // ── 5. profile_battery ──────────────────────────────────────────────────
    server.tool("profile_battery", "Analisis battery drain aplikasi Android.", {
        device_serial: z.string().optional(),
        package_name: z.string(),
        ...
        try {
            ...
            const statsResult = await runAdbCommand(`adb ${flag} shell dumpsys batterystats ${package_name} | head -100`, undefined, 15000);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // ── 6. scan_dependencies ────────────────────────────────────────────────
    server.tool("scan_dependencies", "Scan dependencies untuk mencari known vulnerabilities.", {
        ...
        try {
            ...
            if (build_system === "auto") {
                const files = await fs.readdir(resolvedPath).catch(() => []);
    ...
    // ── 7. extract_secrets ──────────────────────────────────────────────────
    server.tool("extract_secrets", "Scan source code untuk hardcoded secrets.", {
        ...
        try {
            ...
            const secrets = [];
            // Find files
            const findResult = await runCommand(`find "${resolvedPath}" -type f \( -name "*.kt" -o -name "*.java" -o -name "*.js" -o -name "*.ts" -o -name "*.py" \) 2>/dev/null | head -50`, undefined, 10000);
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // ── 8. send_fcm_message ────────────────────────────────────────────────
    server.tool("send_fcm_message", "Kirim push notification test via FCM.", {
        ...
        try {
            ...
            const result = await makeHttpRequest("https://fcm.googleapis.com/fcm/send", "POST", { "Content-Type": "application/json", "Authorization": `key=${apiKey}` }, JSON.stringify({ to: device_token, notification: { title, body } }), 30000);
            const lines = [
    ...
    server.tool("simulate_deep_link", "Simulate deep link di emulator.", {
        device_serial: z.string().optional(),
    ...
    server.tool("test_notification_ui", "Test notification UI - screenshot panel atau list notifications.", {
        device_serial: z.string().optional(),
        ...
        try {
            ...
            if (action === "screenshot_panel") {
                ...
                const notifResult = await runAdbCommand(`adb ${flag} shell dumpsys notification | head -50`, undefined, 10000);
...
//# sourceMappingURL=advanced_testing.js.map