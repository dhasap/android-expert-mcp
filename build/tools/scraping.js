 ...
 */
async function launchBrowserGuarded(options) {
...
export function registerScrapingTools(server) {
    ...
    server.tool("extract_dom_structure", "Extracts a structured summary of a webpage's DOM: headings hierarchy, " +
    ...
    }, async ({ url, include_links, include_images, include_forms, include_meta, include_text, stealth_mode, timeout_seconds, }) => {
        let browser = null;
        let releaseSemaphore = null;
        try {
            const { browser: b, page, releaseSemaphore: rs } = await launchBrowserGuarded({
            ...
            const domData = await page.evaluate((opts) => {
                const data = {};
                // Title & basic info
            ...
            const output = `🔍 DOM Structure: ${url}\n${"─".repeat(60)}\n\n` +
    ...
    server.tool("execute_js_on_page", "Navigates to a URL and executes custom JavaScript on the page, returning " +
        ...
        js_code: z
            .string()
            .describe("JavaScript code to execute. Must return a serializable value. " +
        ...
        try {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: `⚡ JS Execution Result on ${url}\n${"─".repeat(60)}\n\n` +
    ...
    server.tool("monitor_network_requests", "Loads a URL and captures all network requests made by the page: " +
