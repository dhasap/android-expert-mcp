 ...
 */
export async function runCommand(command, cwd, timeoutMs = 120_000) {
 ...
 */
export async function runStreamingCommand(args, cwd, timeoutMs = 300_000) {
    return new Promise((resolve) => {
        const [cmd, ...rest] = args;
        const stdoutChunks = [];
        const stderrChunks = [];
 ...
 */
export async function buildDirectoryTree(dirPath, indent = "", maxDepth = 5, currentDepth = 0) {
    ...
    try {
        ...
        return indent + `  [ERROR reading dir: ${err.message}]\n`;
    }
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            result += await buildDirectoryTree(path.join(dirPath, entry.name), indent + "  ", maxDepth, currentDepth + 1);
        }
    }
    return result;
}
 ...
 */
export async function safeReadFile(filePath, maxSizeBytes = 1_000_000) {
...
export class Semaphore {
    ...
    constructor(concurrency) {
        if (concurrency < 1)
            throw new RangeError("Semaphore concurrency must be >= 1");
 ...
 */
export function buildPuppeteerLaunchOptions(extraArgs = [], overrides = {}) {
 ...
 */
export async function runAdbCommand(command, cwd, timeoutMs = 60_000) {
    const release = await adbMutex.acquire();
    try {
 ...
 */
export function formatSecureToolError(toolName, error, extraSecrets = []) {
 ...
 * or cannot be parsed.
 */
export async function atomicReadJson(filePath, defaultValue) {
    try {
 ...
 */
export function extractStackTrace(output) {
...
export function truncateOutput(text, maxChars = 50_000) {
 ...
 */
export async function cleanupTempDirectories(maxAgeHours = 24) {
    ...
    for (const dirName of MCP_TEMP_DIRS) {
        ...
        try {
            ...
            results.push(`⚠️  ${dirName}: readdir failed — ${err.message}`);
            continue;
        }
        let deleted = 0;
        let freedBytes = 0;
 ...
 */
export function isSafePath(targetPath, allowedRoot) {
